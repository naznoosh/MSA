...
Config::Config(int argc, const char **argv)
{
	...
	parser.add_command("makedb", "Build DIAMOND database from a FASTA file")
		...
		.add_command("dbinfo", "")
		.add_command("malign", "Alignment of all Sequences in FASTA file");
...
("quiet", 0, "disable console output", quiet);
	...
	aligner.add()
		...
		("masking", 0, "enable masking of low complexity regions (0/1=default)", masking, 0/*1*/)
	...
	Options_group malign_options("Compatible Chain MSA Options");
	malign_options.add()
		("minimizer", 0, "minimizer seed", minimizer, false)
		("alphabet-reduction", 0, "reduce alphabet in protein input data sequences", alphabet_reduction, false)
		("seed-pattern", 0, "seed pattern", seed_pattern)
		("package", 0, "supported MSA packages in alignment of subsequences (halign,mafft,muscle,clustalw,clustalo,famsa,kalign)", pkg_name, string("NOALIGN"))
		("run-parallel", 0, "number of parallel pakagess", run_parallel);


	parser.add(general).add(makedb).add(aligner).add(advanced).add(view_options).add(getseq_options).add(hidden_options).add(malign_options);
...
/*		
		...
		{
			char * pch;
			pch = strtok((char *)threads.c_str(), "-");
			if (pch != NULL)
			{
				threads_ = std::stoi(pch);
				pch = strtok(NULL, "");
				if (pch != NULL) parallel = std::stoi(pch);
			}
			if (threads_ == 0) threads_ = parallel = 1;
			else if (parallel == 0) parallel = threads_;
		}
		catch(...)
		{
			string s = "Invalid --threads parameter.\nthe valid format is either\n--threads #\nor\n--threads #-#";
			throw std::runtime_error(s.c_str());
		}
*/
		set_option(run_parallel, threads_);
		...
		std::transform(pkg_name.begin(), pkg_name.end(), pkg_name.begin(), ::toupper);
		if (pkg_name == "NOALIGN") pkg = NOALIGN;
		else if (pkg_name == "MAFFT") pkg = MAFFT;
		else if (pkg_name == "HALIGN") pkg = HALIGN;
		else if (pkg_name == "MUSCLE") pkg = MUSCLE;
		else if (pkg_name == "CLUSTALW") pkg = CLUSTALW;
		else if (pkg_name == "CLUSTALO") pkg = CLUSTALO;
		else if (pkg_name == "FAMSA") pkg = FAMSA;
		else if (pkg_name == "KALIGN") pkg = KALIGN;
		else throw std::runtime_error("Invalid value for parameter --package");

		if (seed_pattern.length() > 0)
		{
			if (seed_pattern.find_first_not_of("01") != string::npos)
				throw std::runtime_error("Invalid value for parameter --seed-pattern\nthe seed pattern must be a string of 0's and 1's");

//			seed_pattern.erase(0, seed_pattern.find_first_not_of('0'));
//			seed_pattern.erase(seed_pattern.find_last_not_of('0') +1 );

			size_t weight = 0;
			for (size_t k = 0; k < seed_pattern.length(); k++)
			{
				if (seed_pattern[k] == '1') weight++;
			}
			if (weight >= Const::max_seed_weight || weight < 5)
			{
				string s = "Invalid value for parameter --seed-pattern\nthe number of 1's in a valid seed must be between 5 and ";
				s.append(to_string(Const::max_seed_weight - 1));
				throw std::runtime_error(s.c_str());
			}

//			if(weight == seed_pattern.length()) seed_pattern.append("0");
		}
	}
}
